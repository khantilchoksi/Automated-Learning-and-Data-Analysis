...
"""
ALDA Fall 2018
HW1 Question#5: sample solution (STANDARDIZED DATASET)
"""
import scipy
import numpy as np
import pandas as pd
...
def Standardize(y_raw):
    ...
    print('Mean: %f, StandardDeviation: %f' % (scaler.mean_, sqrt(scaler.var_)))#standardiz
    standardized = scaler.transform(values)
    y_stand = []
    for each in standardized:
        y_stand.extend(np.round(each,4))
...
data[xlabel] = Standardize(data[xlabel])
data[ylabel] = Standardize(data[ylabel])
...
canberra10 = min_dist(canberra,'canberra') 
